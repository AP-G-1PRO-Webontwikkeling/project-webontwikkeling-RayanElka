...
function filterByID(id: string) {
    const filteredPokemon = data.find((pokemon: Pokemon) => pokemon.id === parseInt(id));
    if (filteredPokemon) {
        ...
        console.log(`  - Evolution Chain: ${filteredPokemon.evolutionChain.baseForm} -> ${filteredPokemon.evolutionChain.evolvesTo} -> ${filteredPokemon.evolutionChain.finalForm}`);
    } else {
